...
class HierarchicalRewardManager:
    ...
    def __init__(self, faction_id: str):
        ...
        self.coordination_history = deque(
            maxlen=utils_config.EXPERIENCE_REPORTING_CONFIG["coordination_window"]
        )
        self.adaptation_history = deque(
            maxlen=utils_config.EXPERIENCE_REPORTING_CONFIG["adaptation_window"]
        )
    ...
    def calculate_agent_reward(
        ...
        efficiency_reward = (
            efficiency_score * weights[utils_config.RewardComponent.EFFICIENCY]
        )
        coordination_reward = (
            coordination_score * weights[utils_config.RewardComponent.COORDINATION]
        )
        adaptation_reward = (
            adaptation_score * weights[utils_config.RewardComponent.ADAPTATION]
        )
        survival_reward = (
            survival_score * weights[utils_config.RewardComponent.SURVIVAL]
        )
    ...
    def _calculate_hq_guidance_bonus(self, agent_id: str, task_type: str) -> float:
        ...
        scaling = utils_config.HIERARCHICAL_REWARD_CONFIG["scaling"][
            "hq_to_agent_guidance"
        ]
    ...
    def _calculate_strategy_selection_reward(
        ...
        progress_bonus = 0.0
        if (
            strategy == "COLLECT_GOLD"
            and mission_progress.get("resource_collection", 0) < 0.5
        ):
            progress_bonus = 0.3
        elif (
            strategy == "ATTACK_THREATS"
            and mission_progress.get("threat_elimination", 0) < 0.8
        ):
            progress_bonus = 0.4
        elif (
            strategy == "DEFEND_HQ"
            and mission_progress.get("territory_control", 0) < 0.6
        ):
            progress_bonus = 0.2
    ...
    def _calculate_strategy_execution_reward(
        ...
        if execution_success:
            ...
            avg_agent_performance = (
                np.mean(list(agent_performance.values())) if agent_performance else 0.0
            )
    ...
    def _calculate_mission_success_reward(self, mission_progress: Dict) -> float:
        ...
        total_progress = (
            np.mean(list(mission_progress.values())) if mission_progress else 0.0
        )
    ...
    def get_adaptation_score(self, agent_id: str, window_size: int = 5) -> float:
        ...
        recent_adaptations = [
            entry for entry in self.adaptation_history if entry["agent_id"] == agent_id
        ][-window_size:]
        ...
        for adapt in recent_adaptations:
            ...
            if "adaptive_strategy_success" in data:
                adaptation_scores.append(
                    1.0 if data["adaptive_strategy_success"] else 0.0
                )
    ...
    def get_efficiency_score(
        ...
        distance_efficiency = max(
            0.0, 1.0 - (distance / 100.0)
        ...
        time_efficiency = max(
            0.0, 1.0 - (time_taken / (expected * 2))
        )  # Allow up to 2x expected time
    ...
    def get_survival_score(
        self, agent_id: str, health: float, max_health: float = 100.0
    ) -> float:
        """
    ...
    def get_reward_summary(self) -> Dict:
        ...
        summary = {
            ...
            "total_agent_experiences": sum(
                len(exps) for exps in self.agent_experiences.values()
            ),
