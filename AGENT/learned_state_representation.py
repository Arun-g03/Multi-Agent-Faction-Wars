...
class StateEncoder(nn.Module):
    ...
    def __init__(self, input_size: int, output_size: int, encoder_type: str = "attention"):
        """
    ...
    def _build_autoencoder(self):
        """Build autoencoder."""
        encoder_layers = utils_config.LEARNED_STATE_CONFIG["encoder_types"][utils_config.StateEncoderType.AUTOENCODER]["encoder_layers"]
        decoder_layers = utils_config.LEARNED_STATE_CONFIG["encoder_types"][utils_config.StateEncoderType.AUTOENCODER]["decoder_layers"]
...
class LearnedStateRepresentationSystem:
    ...
    def __init__(self, faction_id: str, input_state_size: int = 29):
        ...
        self.temporal_memory = deque(maxlen=utils_config.LEARNED_STATE_CONFIG["temporal_window"])
        self.spatial_memory = deque(maxlen=100)
        
        # Learning parameters
        self.learning_rate = utils_config.LEARNED_STATE_CONFIG["learning_rate"]
        self.representation_learning_rate = utils_config.LEARNED_STATE_CONFIG["representation_learning_rate"]
    ...
    def _initialize_state_encoders(self):
        ...
        for repr_type in utils_config.StateRepresentationType:
            component_size = utils_config.STATE_REPRESENTATION_COMPONENTS[repr_type.value]
    ...
    def _state_to_vector(self, state: Dict[str, Any]) -> List[float]:
        ...
        vector.extend([
            ...
            float(nearest_resource.get("type") == "gold") if nearest_resource else 0.0,
    ...
    def _update_encoding_quality(self, repr_type: utils_config.StateRepresentationType, representation: torch.Tensor):
    ...
    def _update_pattern_discovery_rate(self, num_patterns: int):
        ...
        for repr_type in utils_config.StateRepresentationType:
            ...
            alpha = 0.1
            new_rate = (1 - alpha) * current_rate + alpha * (1.0 if num_patterns > 0 else 0.0)
    ...
    def _form_concept_for_type(self, repr_type: utils_config.StateRepresentationType, representation: torch.Tensor) -> Dict[str, Any]:
        ...
        if repr_type == utils_config.StateRepresentationType.ABSTRACT_CONCEPTS:
            ...
            concept["dominant_features"] = torch.topk(representation, k=3).indices.tolist()
            concept["feature_strength"] = torch.topk(representation, k=3).values.tolist()
    ...
    def _update_concept_formation_rate(self, num_concepts: int):
        ...
        for repr_type in utils_config.StateRepresentationType:
            ...
            alpha = 0.1
            new_rate = (1 - alpha) * current_rate + alpha * (1.0 if num_concepts > 0 else 0.0)
    ...
    def get_learned_state_reward(self) -> float:
        ...
        if avg_encoding_quality > 0.8:
            base_reward += utils_config.LEARNED_STATE_CONFIG["representation_rewards"]["representation_quality"]
        
        # Bonus for pattern discovery
        if avg_pattern_discovery > 0.5:
            base_reward += utils_config.LEARNED_STATE_CONFIG["representation_rewards"]["pattern_discovery"]
        
        # Bonus for concept formation
        if avg_concept_formation > 0.5:
            base_reward += utils_config.LEARNED_STATE_CONFIG["representation_rewards"]["concept_formation"]
