...
class ExperienceEncoder(nn.Module):
    ...
    def __init__(self, state_size: int, action_size: int, encoding_size: int = 32):
        ...
        self.state_encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(
                64, utils_config.EXPERIENCE_SHARING_CONFIG["state_encoding_size"]
            ),
        )
        ...
        self.action_encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(
                32, utils_config.EXPERIENCE_SHARING_CONFIG["action_encoding_size"]
            ),
        )
        ...
        self.context_encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(
                16, utils_config.EXPERIENCE_SHARING_CONFIG["context_encoding_size"]
            ),
        )
    ...
    def forward(
        self, state: torch.Tensor, action: torch.Tensor, context: torch.Tensor
...
class ExperienceSharingSystem:
    ...
    def __init__(self, faction_id: str, agents: List[Any]):
        ...
        self.shared_experiences = defaultdict(
            list
        )  # agent_id -> list of shared experiences
        self.collective_memory = deque(
            maxlen=utils_config.COLLECTIVE_LEARNING_CONFIG["collective_memory_size"]
        )

        # Learning parameters
        self.shared_learning_rate = utils_config.EXPERIENCE_SHARING_CONFIG[
            "shared_learning_rate"
        ]
        self.experience_weight_decay = utils_config.EXPERIENCE_SHARING_CONFIG[
            "experience_weight_decay"
        ]
        ...
        for agent in agents:
            ...
            self.experience_encoders[agent_id] = ExperienceEncoder(
                ...
                encoding_size=utils_config.EXPERIENCE_SHARING_CONFIG[
                    "experience_encoding_size"
                ],
            )
    ...
    def encode_experience(
        ...
        context_values = [
            ...
            (
                coordination_data.get("coordination_score", 0.0)
    ...
    def should_share_experience(
        self, agent_id: str, experience_metadata: Dict[str, Any]
    ) -> bool:
        """
    ...
    def find_similar_experiences(
        self, agent_id: str, experience_encoding: torch.Tensor
    ) -> List[Dict[str, Any]]:
        """
        ...
        """
        similar_experiences = []
        similarity_threshold = utils_config.EXPERIENCE_SHARING_CONFIG[
        ...
        for experience in self.collective_memory:
            if experience["agent_id"] != agent_id:  # Don't match with self
                similarity = self._calculate_experience_similarity(
                    experience_encoding, experience["encoding"]
                )
        ...
        similar_experiences.sort(
            key=lambda x: x["similarity"] * x["value"], reverse=True
        )
    ...
    def _calculate_experience_similarity(
        ...
        similarity = F.cosine_similarity(
            encoding1.unsqueeze(0), encoding2.unsqueeze(0), dim=1
        )
        ...
        try:
            ...
            for nearby_agent in nearby_agents:
                if nearby_agent.agent_id != agent_id:
                    self.shared_experiences[nearby_agent.agent_id].append(
                        experience_metadata
                    )
                    shared_count += 1
    ...
    def learn_from_shared_experiences(self, agent_id: str) -> float:
        ...
        for experience in shared_experiences[:10]:  # Limit to recent experiences
            try:
                ...
                learning_value = (
                    experience["value"] * 0.1
                )  # Scale down for learning reward
        ...
        if successful_learnings > 0:
            ...
            learning_reward += utils_config.EXPERIENCE_SHARING_CONFIG[
                "sharing_rewards"
    ...
    def _find_nearby_agents(self, agent_id: str, range_distance: float) -> List[Any]:
        ...
        for agent in self.agents:
            if agent != source_agent:
                distance = math.sqrt(
                    (source_agent.x - agent.x) ** 2 + (source_agent.y - agent.y) ** 2
                )
    ...
    def get_collective_learning_reward(self, agent_id: str) -> float:
        ...
        if sharing_rate > 0.8 and learning_rate > 0.8:
            base_reward += utils_config.EXPERIENCE_SHARING_CONFIG["sharing_rewards"][
    ...
    def get_experience_sharing_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_shared_experiences": sum(
                len(exps) for exps in self.shared_experiences.values()
            ),
        }
