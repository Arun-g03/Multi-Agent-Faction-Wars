...
class AgentBehaviour(
    ...
    def perform_task(
        ...
        if task_state in [
            ...
            if task_state == utils_config.TaskState.FAILURE:
                ...
                if (
                    adaptive_params["agent_adaptability"] > 0.3
                ):  # Only if agent is somewhat adaptive
                    # Analyze the failure
                    failure_type = self.analyze_failure(task_state)

                    # Select adaptive strategy
                    adaptive_strategy = self.select_adaptive_strategy(
                        failure_type, adaptive_params
                    )
    ...
    def assign_reward(
        ...
        if hasattr(agent, "faction") and hasattr(
            agent.faction, "hierarchical_reward_manager"
        ):
            ...
            coordination_data = self._get_coordination_data(
                agent, task_type, task_state
            )
        ...
        if utils_config.ENABLE_TENSORBOARD and task_state is not None:
            try:
                ...
                suffix = f"_{agent.role}_{task_type}_{task_state.value if hasattr(task_state, 'value') else str(task_state)}"
        ...
        if hasattr(agent, "faction") and hasattr(agent.faction, "agents"):
            ...
            if other_agents:
                ...
                if hasattr(agent.faction, "learned_communication"):
                    ...
                    coordination_data["communication_success_rate"] = (
                        learned_comm.communication_success_rate.get(agent.agent_id, 0.5)
                    )
                    coordination_data["coordination_success_rate"] = (
                        learned_comm.coordination_success_rate.get(agent.agent_id, 0.5)
                    )

                    # Check if agent has pending messages
                    if agent.agent_id in learned_comm.message_queues:
                        pending_messages = len(
                            learned_comm.message_queues[agent.agent_id]
                        )
                    ...
                    if agent.agent_id in learned_comm.communication_history:
                        recent_communications = learned_comm.communication_history[
                            agent.agent_id
                        ][
                            -5:
                        ...
                        successful_communications = sum(
                            ...
                            if comm.get("success", False)
                        )
                        coordination_data["recent_communication_success"] = (
                            successful_communications / len(recent_communications)
                ...
                if hasattr(agent.faction, "experience_sharing"):
                    ...
                    coordination_data["sharing_success_rate"] = (
                        exp_sharing.sharing_success_rate.get(agent.agent_id, 0.5)
                    )
                    coordination_data["learning_success_rate"] = (
                        exp_sharing.learning_success_rate.get(agent.agent_id, 0.5)
                    )

                    # Check if agent has shared experiences
                    if agent.agent_id in exp_sharing.shared_experiences:
                        shared_experiences = len(
                            exp_sharing.shared_experiences[agent.agent_id]
                        )
                        coordination_data["shared_experiences"] = shared_experiences

                    # Check collective memory size
                    coordination_data["collective_memory_size"] = len(
                        exp_sharing.collective_memory
                    )
    ...
    def _get_adaptation_data(self, agent, task_type, task_state) -> Dict:
        ...
        if hasattr(agent, "adaptive_strategy_used"):
            ...
            adaptation_data["adaptive_strategy_success"] = getattr(
                agent, "adaptive_strategy_success", False
            )

        # Check if agent recovered from failure
        if task_state == utils_config.TaskState.SUCCESS and hasattr(
            agent, "previous_task_state"
        ):
    ...
    def analyze_failure(self, task_state, context=None):
        ...
        if task_state == utils_config.TaskState.FAILURE:
            ...
            if task_type == "gather":
                ...
                for resource in self.agent.faction.resource_manager.resources:
                    if (
                        resource.grid_x,
                        resource.grid_y,
                    ) == target_position and not resource.is_depleted():
                        resource_found = True
                        break
                ...
                current_pos = (
                    int(self.agent.x // utils_config.CELL_SIZE),
                    int(self.agent.y // utils_config.CELL_SIZE),
                )

                if (
                    self.agent.faction.current_step - self.agent.task_start_step > 50
                ):  # Stuck for too long
            ...
            if hasattr(self.agent, "task_start_step"):
                time_elapsed = (
                    self.agent.faction.current_step - self.agent.task_start_step
                )
    ...
    def execute_adaptive_strategy(self, strategy, state, resource_manager, agents):
        ...
        if strategy == utils_config.AdaptiveStrategy.RETRY_WITH_MODIFICATION:
            ...
            )  # Use emergency protocol for retreat
    ...
    def _request_support(self, state, resource_manager, agents):
        ...
        for agent in agents:
            if (
                agent.faction.id == self.agent.faction.id
                and agent.agent_id != self.agent.agent_id
            ):
                distance = (
                    (agent.x - self.agent.x) ** 2 + (agent.y - self.agent.y) ** 2
                ) ** 0.5
    ...
    def _escalate_to_hq(self, state, resource_manager, agents):
        ...
        return (
            utils_config.TaskState.FAILURE
        )  # Current task failed, but HQ will assign new one
    ...
    def _opportunistic_action(self, state, resource_manager, agents):
        ...
        for resource in resource_manager.resources:
            ...
            distance = (
                (resource_pos[0] - current_pos[0]) ** 2
                + (resource_pos[1] - current_pos[1]) ** 2
            ) ** 0.5
    ...
    def _find_alternative_threat(self, agents):
        ...
        for threat in self.agent.faction.global_state.get("threats", []):
            if threat["id"].faction_id != self.agent.faction.id:
                ...
                distance = (
                    (threat_pos[0] - current_pos[0]) ** 2
                    + (threat_pos[1] - current_pos[1]) ** 2
                ) ** 0.5
    ...
    def _get_adaptive_parameters(self):
        ...
        if (
            hasattr(self.agent.faction, "current_strategy_parameters")
            and self.agent.faction.current_strategy_parameters
        ):
            ...
            return {
                ...
                "exploration_tendency": params.get(
                    "mission_autonomy", 0.5
                ),  # Use mission_autonomy as exploration tendency
                "collaboration_willingness": params.get("coordination_preference", 0.5),
                "risk_tolerance": params.get("aggression_level", 0.5),
                "escalation_threshold": 1.0
                - params.get(
                    "urgency", 0.5
                ),  # Higher urgency = lower escalation threshold
            }
        else:
