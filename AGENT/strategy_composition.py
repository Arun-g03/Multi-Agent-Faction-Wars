...
class StrategyComposer(nn.Module):
    ...
    def forward(
        self, state: torch.Tensor, goal_type: torch.Tensor
...
class StrategyCompositionSystem:
    ...
    def __init__(self, faction_id: str, state_size: int = 32):
        ...
        # Learning parameters
        self.composition_learning_rate = utils_config.STRATEGY_COMPOSITION_CONFIG[
            "composition_learning_rate"
        ]
        self.sequence_learning_rate = utils_config.STRATEGY_COMPOSITION_CONFIG[
    ...
    def create_goal(
        self, goal_type: utils_config.StrategyGoalType, priority: float = 0.5
    ) -> StrategyGoal:
        """
    ...
    def compose_strategy(
        self, state: Dict[str, Any], goal: StrategyGoal, current_step: int
    ) -> StrategyComposition:
        """
        ...
        composition_type = list(utils_config.StrategyCompositionType)[
            composition_type_idx
        ]
        ...
        selected_strategies = [
            utils_config.HQ_STRATEGY_OPTIONS[i] for i in top_strategies
        ]
        ...
        sequence = StrategySequence(
            ...
            timeout_step=current_step
            + utils_config.STRATEGY_COMPOSITION_CONFIG["strategy_timeout"],
        )
    ...
    def execute_composition(
        self, composition: StrategyComposition, current_step: int
    ) -> str:
        """
        ...
        if (
            ...
            == utils_config.StrategyCompositionType.SEQUENTIAL
        ):
            return self._execute_sequential(composition, current_step)
        elif (
            composition.composition_type
            == utils_config.StrategyCompositionType.PARALLEL
        ):
            return self._execute_parallel(composition, current_step)
        elif (
            composition.composition_type
            == utils_config.StrategyCompositionType.CONDITIONAL
        ):
            return self._execute_conditional(composition, current_step)
        elif (
            composition.composition_type
            == utils_config.StrategyCompositionType.HIERARCHICAL
        ):
            return self._execute_hierarchical(composition, current_step)
        elif (
            composition.composition_type
            == utils_config.StrategyCompositionType.ADAPTIVE
        ):
    ...
    def update_goal_progress(
        self, goal: StrategyGoal, state: Dict[str, Any], current_step: int
    ):
        """
    ...
    def evaluate_composition_success(
        self, composition: StrategyComposition, current_step: int
    ) -> float:
        """
        ...
        completed_sequences = sum(
            1 for seq in composition.sequences if seq.execution_state == "completed"
        )
        sequence_completion = (
            completed_sequences / len(composition.sequences)
        ...
        success_score = (
            goal_progress * 0.5 + efficiency * 0.3 + sequence_completion * 0.2
        )
    ...
    def get_composition_reward(self) -> float:
        ...
        avg_composition_success = (
            np.mean(list(self.composition_success_rate.values()))
            if self.composition_success_rate
            else 0.0
        )
        ...
        if self.composition_history:
            ...
            avg_efficiency = np.mean(
                [comp.success_rate for comp in recent_compositions]
            )
            efficiency_bonus = avg_efficiency * 0.2

        # Bonus for adaptive behavior
        adaptive_compositions = sum(
            1
            for comp in self.active_compositions.values()
    ...
    def _goal_type_to_tensor(
        self, goal_type: utils_config.StrategyGoalType
    ) -> torch.Tensor:
        """
        Convert goal type to one-hot tensor.
    ...
    def get_composition_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_sequence_success": (
                np.mean(list(self.sequence_success_rate.values()))
                if self.sequence_success_rate
                else 0.0
            ),
            "avg_goal_achievement": (
                np.mean(list(self.goal_achievement_rate.values()))
                if self.goal_achievement_rate
                else 0.0
            ),
        }
