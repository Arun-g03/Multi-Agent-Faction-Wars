...
class CommunicationNetwork(nn.Module):
    ...
    def __init__(self, state_size: int, message_size: int, hidden_size: int = 128):
        ...
        self.communication_decision = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(hidden_size // 2, len(utils_config.CommunicationType) + 1),  # +1 for "no communication"
            nn.Softmax(dim=-1),
        )
    ...
    def decide_communication(self, state: torch.Tensor, message: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
...
class LearnedCommunicationSystem:
    ...
    def __init__(self, faction_id: str, agents: List[Any]):
        ...
        for agent in agents:
            ...
            self.communication_success_rate[agent_id] = 0.5  # Start with neutral success rate
    ...
    def get_communication_state(self, agent: Any) -> torch.Tensor:
        ...
        task_gather = 1.0 if agent.current_task and agent.current_task.get("type") == "gather" else 0.0
        task_eliminate = 1.0 if agent.current_task and agent.current_task.get("type") == "eliminate" else 0.0
        task_defend = 1.0 if agent.current_task and agent.current_task.get("type") == "defend" else 0.0
        ...
        state = torch.tensor([
            ...
            health,                          # Agent health
            role_gatherer, role_peacekeeper, # Agent role
            task_none, task_gather, task_eliminate, task_defend,  # Current task
    ...
    def should_communicate(self, agent: Any, current_step: int) -> bool:
        ...
        if len(self.message_queues[agent_id]) >= utils_config.COMMUNICATION_CONFIG["message_queue_size"]:
            return False
    ...
    def generate_message(self, agent: Any) -> Optional[Dict[str, Any]]:
        ...
        message_dict = {
            ...
            "timestamp": time.time(),
            "expiry_steps": utils_config.COMMUNICATION_CONFIG["message_types"][comm_type]["expiry_steps"],
        }
    ...
    def get_communication_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_communication_success": np.mean(list(self.communication_success_rate.values())),
            "avg_coordination_success": np.mean(list(self.coordination_success_rate.values())),
            "total_messages_sent": sum(len(history) for history in self.communication_history.values()),
        }
