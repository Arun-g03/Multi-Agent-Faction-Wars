...
class GameManager:
    def __init__(
        ...
        self.models_loaded_once = (
            False  # Ensure models are loaded only once at the start
        )
        # Set to True if you want to exit the sim loop and return to menu after
        # an episode
        self.exit_after_episode = False

    def set_mode(
        self,
        # can be 'train', 'evaluate', or a config dict
        mode: Union[str, Dict[str, Any]],
    ...
    def step(self):
        ...
        try:
            ...
            for faction in self.faction_manager.factions:
                ...
                # Get the current strategy for the faction (e.g., "DEFEND_HQ",
                # "COLLECT_GOLD", etc.)
                if faction.current_strategy is not None:
                    strategy_index = utils_config.HQ_STRATEGY_OPTIONS.index(
                        faction.current_strategy
                    )

                    # Use one-hot encoding to visualize which strategy is being
                    # used
                    one_hot_strategy = np.zeros(
                        (1, len(utils_config.HQ_STRATEGY_OPTIONS)), dtype=int
                    )
                    one_hot_strategy[0, strategy_index] = 1

                    # Define the strategy keys for X-axis labeling
                    strategy_keys = (
                        utils_config.HQ_STRATEGY_OPTIONS
                    )  # List of strategies
            ...
            if utils_config.ENABLE_TENSORBOARD:
                ...
                for agent in self.agents:
                    if agent.current_action is not None:
                        ...
                        task_type = (
                            agent.current_task.get("type", "none")
                        ...
                        one_hot_action = np.zeros(
                            (1, len(utils_config.ROLE_ACTIONS_MAP[role])), dtype=int
                        )
                        one_hot_action[0, action_index] = 1

                        # Define the keys (e.g., action names or strategy
                        # names) for X-axis labeling
                        # List of action names for current role
                        action_keys = utils_config.ROLE_ACTIONS_MAP[role]
                        ...
                        # Define the keys (e.g., task names) for X-axis
                        # labeling
                    ...
                    if not agent.has_logged_model and self.current_step == 0:
                        try:
                            real_state = agent.get_state(
                                agent.resource_manager,
                                self.agents,
                                agent.faction,
                                agent.faction.global_state,
                            )
                            state_tensor = (
                                torch.tensor(real_state, dtype=torch.float32)
                                .unsqueeze(0)
                                .to(Training_device)
                            )

                            tensorboard_logger.log_model(
                                agent.ai,
                                state_tensor,
                                name=f"Agent_{agent.agent_id}_Model",
                            )

                            agent.has_logged_model = True
                        except Exception as e:
                            print(
                                f"[TensorBoard] Failed to log model graph for agent {agent.agent_id}: {e}"
                            )
    ...
    def load_models(self):
        ...
        for agent in self.agents:
            ...
            if ckpt and hasattr(agent, "ai"):
                ...
                print(
                    f"[LOAD] {agent.role.capitalize()} {agent.agent_id} restored from {ckpt}"
                )
        ...
        try:
            ...
            while running and (self.episode <= utils_config.EPISODES_LIMIT):
                ...
                self.resource_spawn_history["gold_lumps"].append(
                    self.resource_counts["gold_lumps"]
                )
                self.resource_spawn_history["gold_quantity"].append(
                    self.resource_counts["gold_quantity"]
                )
                self.resource_spawn_history["apple_trees"].append(
                    self.resource_counts["apple_trees"]
                )
                self.resource_spawn_history["apple_quantity"].append(
                    self.resource_counts["apple_quantity"]
                )
                ...
                self.log_faction_metrics(
                    tensorboard_logger, plotter, processed_role_rewards, hq_rewards
                )
                ...
                # Clear agent memories after training to prepare for next
                # episode
                if self.mode == "train":
                    for agent in self.agents:
    ...
    def log_faction_metrics(
        self, tensorboard_logger, plotter, role_rewards, hq_rewards
    ):
        if utils_config.ENABLE_TENSORBOARD:
            tensorboard_logger.log_scalar(
                "Episode/Steps_Taken", self.current_step, self.episode
        ...
        for faction in self.faction_manager.factions:
            ...
            average_reward = (
                faction_total_reward / agent_count if agent_count > 0 else 0
            )
            ...
            if hq_rewards is not None:
                ...
                self.metric_history[faction.id]["hq_reward"].append(
                    hq_rewards[faction.id]
                )
            ...
            faction_rewards = []
            # Define the roles you want to plot (adjust as necessary)
            role_names = ["gatherer", "peacekeeper"]
            ...
            plotter.plot_scalar_over_time(
                names=[
                    f"Faction {faction.id} Gatherer Reward",
                    f"Faction {faction.id} Peacekeeper Reward",
                    f"Faction {faction.id} HQ Reward",
                ],
            ...
            if hasattr(faction, "assigned_tasks") and faction.assigned_tasks:
                # For the current episode, gather the success, failure, and
                # ongoing counts for each task type
                task_records = faction.assigned_tasks

                # Initialise counts for success, failure, ongoing tasks per
                # task type per episode
                task_types = [
                    "mine",
                    "forage",
                    "explore",
                    ...
                    "eliminate",
                    "move_to",
                ]  # Define task types you want to categorize
                ...
                for task_type in task_types:
                    ...
                    for task_id, task_info in task_records.items():
                        if (
                            task_info.get("type", "") == task_type
                        ):  # Directly check task type instead of task_id
                ...
                # Plot the clustered stacked bar chart for each faction in the
                # current episode
                plotter.plot_clustered_stacked_bar_chart(
                    ...
                    tensorboard_logger=tensorboard_logger,
                    step=self.current_step,
                    # Use faction ID to name the plot
                    name=f"Faction {faction.id} Task Timeline {self.episode}",
                )
    ...
    def save_top_models(self, role_rewards):
        ...
        for role, reward_list in role_rewards.items():
            ...
            model_files = [
                (
                    os.path.join(agent_model_dir, f),
            ...
            if (None, best_reward) in top5:
                save_name = (
                    f"Best_{role}_episode_{self.episode}_reward_{best_reward:.2f}.pth"
                )
            ...
            if len(model_files) > 5:
                ...
                for f in os.listdir(agent_model_dir):
                    ...
                    if (
                        ...
                        and f.startswith(f"Best_{role}_")
                        and "_reward_" in f
                    ):
                        os.remove(f_path)
                        if utils_config.ENABLE_LOGGING:
                            self.logger.log_msg(
                                f"[PRUNE] Removed outdated model: {f_path}",
                                level=logging.INFO,
                            )
    ...
    def train_agents(self):
        ...
        if utils_config.ENABLE_LOGGING:
            self.logger.log_msg(
                "[TRAINING] Training agents at end of episode.", level=logging.INFO
            )
    ...
    def train_agents_mini(self):
        ...
        for agent in self.agents:
            if (
                agent.mode == "train"
                and len(agent.ai.memory["rewards"]) >= utils_config.MIN_MEMORY_SIZE
            ):
                try:
        ...
        for faction in self.faction_manager.factions:
            ...
            if hasattr(faction.network, "train") and faction.network.hq_memory:
                try:
                    ...
                    combined_scores = (
                        model_files + self.global_hq_top5 + [(None, hq_reward)]
                    )
                    combined_scores = list(
                        {(p, r) for p, r in combined_scores}
                    )  # deduplicate
                    ...
                    if (None, hq_reward) in top5:
                        ...
                        if hasattr(faction.network, "save_model"):
                            ...
                            print(
                                f"[SAVE] HQ model for Faction {faction.id} saved at {save_path}"
                            )
    ...
    def print_episode_summary(self):
        ...
        if utils_config.ENABLE_LOGGING:
            self.logger.log_msg(
                f"End of {self.mode} Episode {self.episode}", level=logging.INFO
            )
    ...
    def handle_victory(self, winner):
        ...
        if utils_config.ENABLE_LOGGING:
            self.logger.log_msg(
                f"Faction {winner.id} wins! Ending episode early.", level=logging.INFO
            )
