...
class PPOModel(nn.Module):
    ...
    def choose_action(self, state, valid_indices=None):
        ...
        if utils_config.ENABLE_LOGGING:
            logger.log_msg(
                f" AgentID: {self.AgentID}__State: {state}", level=logging.DEBUG
            )
        ...
        if torch.isnan(probs).any().item() or torch.isinf(probs).any().item():
            logger.log_msg(
                "[PROBS ERROR] Detected NaNs/Infs in probs!", level=logging.ERROR
            )
    ...
    def store_transition(
        self, state, action, log_prob, reward, local_value, global_value, done
    ):
        """
        ...
        if utils_config.ENABLE_LOGGING:
            logger.log_msg(
                f"AGENT - Storing transition: state={state}"
                f"action={action}"
                f"reward={reward}"
    ...
    def train(self, mode="train", batching=False):
        ...
        try:
            ...
            if mode == "train":
                ...
                if utils_config.ENABLE_LOGGING:
                    logger.log_msg(
                        "[EVALUATE MODE] No training applied.", level=logging.DEBUG
                    )
            else:
                if utils_config.ENABLE_LOGGING:
                    logger.log_msg(
            ...
            if utils_config.ENABLE_LOGGING:
                logger.log_msg(
                    f"[ERROR] Exception in PPOAgent.train: {e}", level=logging.ERROR
                )
    ...
    def train_batched(self):
        ...
        actions = torch.tensor(
            [self.memory["actions"][i] for i in indices],
            dtype=torch.long,
            device=device,
        )
        old_log_probs = torch.tensor(
            [self.memory["log_probs"][i] for i in indices],
            dtype=torch.float32,
            device=device,
        )
        rewards = torch.tensor(
            [self.memory["rewards"][i] for i in indices],
            dtype=torch.float32,
            device=device,
        )
        values = torch.tensor(
            [self.memory["values"][i] for i in indices],
            dtype=torch.float32,
            device=device,
        )
        dones = torch.tensor(
            [self.memory["dones"][i] for i in indices],
            dtype=torch.float32,
            device=device,
        )
        ...
        if torch.isnan(logits).any():
            if utils_config.ENABLE_LOGGING:
                logger.log_msg(
                    f"[TRAIN LOGITS ERROR] NaNs in logits during training",
                    level=logging.ERROR,
                )
            return
        ...
        if torch.isnan(probs).any():
            if utils_config.ENABLE_LOGGING:
                logger.log_msg(
                    f"[TRAIN PROBS ERROR] NaNs in probs during training",
                    level=logging.ERROR,
                )
            return
        ...
        surr2 = (
            torch.clamp(
                ratio, 1 - utils_config.PPO_CLIP_RATIO, 1 + utils_config.PPO_CLIP_RATIO
            )
            * advantages
        )
        ...
        loss = (
            ...
            - utils_config.ENTROPY_COEFF * entropy
        )
    ...
    def train_individual(self):
        ...
        old_log_probs = torch.tensor(
            self.memory["log_probs"], dtype=torch.float32, device=device
        )
        rewards = torch.tensor(
            self.memory["rewards"], dtype=torch.float32, device=device
        )
    ...
    def compute_gae(self, rewards, local_values, global_values, dones):
        ...
        if (
            len(rewards) == 0
            or len(local_values) == 0
            or len(global_values) == 0
            or len(dones) == 0
        ):
            if utils_config.ENABLE_LOGGING:
                logger.log_msg(
        ...
        if utils_config.ENABLE_LOGGING:
            logger.log_msg(
                f"[GAE] Computing GAE for {len(rewards)} transitions",
                level=logging.DEBUG,
            )
        ...
        try:
            ...
            if not isinstance(local_values, torch.Tensor):
                local_values = torch.tensor(
                    local_values, dtype=torch.float32, device=self.device
                )
            ...
            for t in reversed(range(len(rewards))):
                ...
                delta = (
                    rewards[t] + gamma * next_value * (1 - dones[t]) - local_values[t]
                )
            ...
            if utils_config.ENABLE_LOGGING:
                logger.log_msg(
                    f"[ERROR] GAE computation failed: {e}", level=logging.ERROR
                )
    ...
    def reset_model(self, keep_memory=False):
        ...
        if utils_config.ENABLE_LOGGING:
            logger.log_msg(
                f"[MODEL RESET] PPO model reset successfully. Memory preserved: {keep_memory}",
                level=logging.INFO,
            )
    ...
    def adaptive_entropy_coefficient(self, episode_reward, target_entropy=0.1):
        ...
        if abs(new_entropy_coeff - self.entropy_coeff) > 0.001:
            ...
            if utils_config.ENABLE_LOGGING:
                logger.log_msg(
                    f"[ADAPTIVE ENTROPY] Adjusted entropy coefficient to {self.entropy_coeff:.4f}",
                    level=logging.DEBUG,
                )
    ...
    def get_model_complexity(self):
        ...
        return {
            ...
            "architecture": (
                "Enhanced PPO with Attention" if self.attention else "Enhanced PPO"
            ),
        }
    ...
    def set_learning_rate(self, new_lr):
        ...
        if utils_config.ENABLE_LOGGING:
            logger.log_msg(
                f"[LR UPDATE] Learning rate updated to {new_lr:.6f}", level=logging.INFO
            )
    ...
    def freeze_layers(self, layer_names=None):
        ...
        for name, param in self.named_parameters():
            if any(layer_name in name for layer_name in layer_names):
                ...
                if utils_config.ENABLE_LOGGING:
                    logger.log_msg(
                        f"[FREEZE] Frozen layer: {name}", level=logging.DEBUG
                    )
        ...
        if layer_names is None:
            ...
            for name, param in self.named_parameters():
                if any(layer_name in name for layer_name in layer_names):
                    ...
                    if utils_config.ENABLE_LOGGING:
                        logger.log_msg(
                            f"[UNFREEZE] Unfrozen layer: {name}", level=logging.DEBUG
                        )

        if utils_config.ENABLE_LOGGING:
            logger.log_msg(
                f"[UNFREEZE] Unfrozen layers: {layer_names if layer_names else 'all'}",
                level=logging.INFO,
            )
