...
class AgentBehaviour:
    def __init__(self, agent, state_size, action_size, role_actions, event_manager):
        ...
        self.role_actions = utils_config.ROLE_ACTIONS_MAP[agent.role]  # Get valid actions for the agent's role
        self.current_task = None  # Initialise the current task

        
        self.target = None
        if utils_config.ENABLE_LOGGING: logger.log_msg(f"initialised behavior for {self.agent.role} with actions: {self.role_actions}.", level=logging.DEBUG)
...
#         |___/                                                                      
    ...
    def perform_action(self, action_index, state, resource_manager, agents):
        ...
        """
        if utils_config.ENABLE_LOGGING: logger.log_msg(f"Starting perform_action for agent {self.agent.role}.", level=logging.DEBUG)
        
        role_actions = utils_config.ROLE_ACTIONS_MAP[self.agent.role]
        action = role_actions[action_index]

        if utils_config.ENABLE_LOGGING: logger.log_msg(f"Action selected: {action}. Validating action existence.", level=logging.DEBUG)

        if hasattr(self, action):
            if utils_config.ENABLE_LOGGING: logger.log_msg(f"Action '{action}' found. Dynamically determining arguments.", level=logging.DEBUG)
        ...
        if utils_config.ENABLE_LOGGING: logger.log_msg(f"Action '{action}' not implemented for agent {self.agent.role}.", level=logging.WARNING)
...
#                                                                                                                               
    ...
    def assign_reward_for_independent_action(self, task_state):
        ...
        # Add TensorBoard logging here too
            try:
                ...
                if utils_config.ENABLE_LOGGING: logger.log_msg(f"[TensorBoard] Failed to log independent task reward: {e}", level=logging.WARNING)
    ...
    def assign_reward_for_task_action(self, task_state, task_type, agent, target_position, current_position):
        ...
        if utils_config.ENABLE_TENSORBOARD:
            try:
                ...
                if utils_config.ENABLE_LOGGING: logger.log_msg(f"[TensorBoard] Failed to log task reward: {e}", level=logging.WARNING)
    ...
    def handle_eliminate_task(self, target, agents):
        ...
        """
        if utils_config.ENABLE_LOGGING: logger.log_msg(f"Agent {self.agent.role} received eliminate task for target: {target}.", level=logging.INFO)

        if not target or "position" not in target:
            if utils_config.ENABLE_LOGGING: logger.log_msg(f"Invalid eliminate task target: {target}.", level=logging.WARNING)
        ...
        if self.agent.is_near(target_position):
            if utils_config.ENABLE_LOGGING: logger.log_msg(f"{self.agent.role} is in range to eliminate target. Executing eliminate_threat.", level=logging.INFO)
    ...
    def handle_gather_task(self, state, resource_manager, agents):
        ...
        if utils_config.ENABLE_LOGGING: logger.log_msg(f"{self.agent.role} handling gather task. Target position: {target_position}", level=logging.INFO)
        ...
        if not self.agent.is_near((resource_obj.x, resource_obj.y), threshold=3):
            ...
            if utils_config.ENABLE_LOGGING: logger.log_msg(f"{self.agent.role} moving towards resource at ({resource_obj.x}, {resource_obj.y}) (dx: {dx}, dy: {dy}).", level=logging.INFO)
            return self.move_to_target(dx, dy)

        # Gather from the object
        if hasattr(resource_obj, "gather") and callable(resource_obj.gather):
            if utils_config.ENABLE_LOGGING: logger.log_msg(f"{self.agent.role} gathering from resource at {target_position}.", level=logging.INFO)
            ...
            if utils_config.ENABLE_LOGGING: logger.log_msg(f"{self.agent.role} mining gold at {target_position}.", level=logging.INFO)
            resource_obj.mine()
    ...
    def handle_explore_task(self):
        ...
        """
        if utils_config.ENABLE_LOGGING: logger.log_msg(f"{self.agent.role} executing explore task.", level=logging.INFO)

        unexplored_cells = self.find_unexplored_areas()
        if unexplored_cells:
            # Select a random unexplored cell
            target_cell = random.choice(unexplored_cells)
            target_position = (target_cell[0] * utils_config.CELL_SIZE, target_cell[1] * utils_config.CELL_SIZE)
        ...
        if utils_config.ENABLE_LOGGING: logger.log_msg(f"{self.agent.role} found no unexplored areas.", level=logging.WARNING)
        return utils_config.TaskState.FAILURE
...
#     
    ...
    def heal_with_apple(self):
        ...
        if self.agent.Health < 100 and self.agent.faction.food_balance > 0:
            ...
            if utils_config.ENABLE_LOGGING: logger.log_msg(f"{self.agent.role} healed with an apple. Health is now {self.agent.Health}.", level=logging.INFO)
            
        else:
            if utils_config.ENABLE_LOGGING: logger.log_msg(f"{self.agent.role} attempted to heal, but no food available.", level=logging.WARNING)
    ...
    def explore(self):
        ...
        if self.agent.current_task.get("target") is None:
            if utils_config.ENABLE_LOGGING: logger.log_msg(f"[EXPLORE] No target set for agent {self.agent.agent_id}", level=logging.WARNING)
            return utils_config.TaskState.INVALID
        
        

        target_x, target_y = self.agent.current_task["target"]["position"]
        world_x, world_y = target_x * utils_config.CELL_SIZE, target_y * utils_config.CELL_SIZE
        ...
        if abs(dx) <= threshold and abs(dy) <= threshold:
            if utils_config.ENABLE_LOGGING: logger.log_msg(f"[EXPLORE COMPLETE] Agent {self.agent.agent_id} reached ({target_x}, {target_y})", level=logging.INFO)
            return utils_config.TaskState.SUCCESS
        ...
        if hasattr(self, action):
            getattr(self, action)()
            if utils_config.ENABLE_LOGGING: logger.log_msg(f"[EXPLORE] Agent {self.agent.agent_id} exploring toward ({target_x}, {target_y}) via '{action}'", level=logging.DEBUG)
            return utils_config.TaskState.ONGOING
        else:
            if utils_config.ENABLE_LOGGING: logger.log_msg(f"[EXPLORE ERROR] Agent {self.agent.agent_id} cannot perform '{action}'", level=logging.WARNING)
            return utils_config.TaskState.FAILURE
    ...
    def find_unexplored_areas(self):
        ...
        if utils_config.ENABLE_LOGGING: logger.log_msg(f"{self.agent.role} identified unexplored areas: {unexplored}.", level=logging.DEBUG)
        return unexplored
...
#                                                                        
    ...
    def forage_apple(self):
        ...
        if apple_trees:
            ...
            if self.agent.is_near(tree, utils_config.Agent_Interact_Range*utils_config.CELL_SIZE):
        ...
        if utils_config.ENABLE_LOGGING: logger.log_msg(f"{self.agent.role} found no apple trees nearby to forage.", level=logging.WARNING)
        return utils_config.TaskState.FAILURE
...
#             
    ...
    def patrol(self):
        ...
        if threat:
            ...
            if utils_config.ENABLE_LOGGING: logger.log_msg(f"{self.agent.role} found no threats to patrol towards.", level=logging.WARNING)
...
#                                                                    
    ...
    def eliminate_threat(self, agents):
        ...
        task = self.agent.current_task
        if not task:
            if utils_config.ENABLE_LOGGING: logger.log_msg(f"{self.agent.role} has no valid task assigned for elimination.", level=logging.WARNING)
            return utils_config.TaskState.FAILURE

        threat = task.get("target")
        if not threat or "position" not in threat:
            if utils_config.ENABLE_LOGGING: logger.log_msg(f"{self.agent.role} could not find a valid threat in the task.", level=logging.WARNING)
            return utils_config.TaskState.FAILURE
        ...
        if self.agent.is_near(assigned_position, threshold=utils_config.Agent_Interact_Range*utils_config.CELL_SIZE):
        ...
        # Step 2: Attack any other enemy agent within combat range
            for enemy in agents:
                if (
                    ...
                    self.agent.is_near((enemy.x, enemy.y), threshold=utils_config.Agent_Interact_Range*utils_config.CELL_SIZE)
                ):
        ...
        if utils_config.ENABLE_LOGGING: logger.log_msg(f"{self.agent.role} could not reach assigned threat and found no enemies in attack range.", level=logging.INFO)
        return utils_config.TaskState.FAILURE
    ...
    def get_agent_by_location(self, location, agents):
        ...
        if not agents or not isinstance(agents, list):
            if utils_config.ENABLE_LOGGING: logger.log_msg(f"'agents' is invalid or missing. Cannot find agent by location.", level=logging.WARNING)
            return None
