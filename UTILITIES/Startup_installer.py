...
def extract_package_info(
    ...
    match = re.match(
        r"^([a-zA-Z0-9_-]+)\s*([<=>!~]+)\s*(.+)$", requirement_line.strip()
    )
    if match:
...
def check_version_compatibility(
    required_version: str, installed_version: str, operator: str
) -> bool:
    """
    ...
    try:
        ...
        if operator == "==":
            ...
            return inst_ver >= req_ver and inst_ver < version.parse(
                f"{req_ver.major + 1}.0.0"
            )
        else:
        ...
        print(
            f"[WARNING] Could not check version compatibility for {required_version}: {e}"
        )
        return True  # Can't check, assume compatible
    ...
    try:
        ...
        for i, line in enumerate(all_lines):
            ...
            if check_package_importable(package_name, import_name):
                ...
                if installed_version:
                    ...
                    if version and not check_version_compatibility(
                        version, installed_version, operator
                    ):
                        ...
                        print(
                            f"  ⚠ Version mismatch: {operator}{version} vs {installed_version}"
                        )
                    else:
        ...
        if version_mismatches:
            ...
            for mismatch in version_mismatches:
                print(
                    f"  {mismatch['package']}: {mismatch['operator']}{mismatch['required']} required, {mismatch['installed']} installed"
                )
